...
형변환 TO_CHAR
EXTRACT : 날짜뽑기
숫자관련함수 CEIL ROUND FLOOR TRUNC AVS MOD
...
2) 0 : 숫자 한 칸 의미, 오른쪽 정려르 빈칸에 0을 추가
3) L : 현재 시스템이나 DB에 설정된 나라의 화폐 기회
4) , : 숫자의 자릿수 구분 
*/
...
-- DUAL 테이블 사용해서 현재 날짜 (SYSDATE) YYYY-MM-DD 수요일
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD DAY')
FROM DUAL;

-- DUAL 테이블 사용해서 현재 날짜 (SYSDATE) YYYY-MM-DD (수)
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD (DY)')
FROM DUAL;

-- DUAL 테이블 사용해서 현재 날짜 (SYSDATE) YYYY-MM-DD (수) 오전 시 : 초
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD (DY) AM HH:MI:SS')
FROM DUAL;

-- DUAL 테이블 사용해서 현재 날짜 (SYSDATE) YYYY월 MM월 DD일 오전 HH시 MI분 SS초
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY"년 "MM"월 "DD"일 " AM HH"시 "MI"분 "SS"초"')
FROM DUAL;

SELECT SYSDATE, TO_CHAR(SYSDATE, 'DL AM HH"시" MI"분" SS"초"')
FROM DUAL;
...
SELECT TO_DATE('05월 27일 월요일 12시 50분', 'MM"월" DD"일" DAY HH"시"MI"분"SS"초"')
FROM DUAL; -- TO_DATE 형식으로 년/월/일 변경하겠다 05월 27일 월요일 12시 50분 -> 24/05/27

-- 숫자 -> 날짜
SELECT T0_DATE(20240605, 'YYYYMMDD') FROM DUAL; --> 24/06/05
...
---│
-- └ 50 이하 : Y, R 둘 다 누락된 연도 앞부분에 현재 2000년대 세기 추가
