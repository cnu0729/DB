...
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY NOT BETWEEN 3000000 AND 3000000;
...
-- EMPLOYEE 에서 이름이 전을 시작하고 돈으로 끝나는 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE
...
SELECT 문의 조회 결과 (RESULT SET)를 정렬할 때 사용하는 구분
...
-- EMP_NAME :현재 2번째 자리에 위치하는 순서를 이용해서 ORDER BY 구할 수 있음
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY BETWEEN 3000000 AND 6000000
ORDER BY 2 DESC; --2 = EMP_NAME
-- 만약에 EMP_ID로 ORDER BY 에서 정렬하길 원한다면 1을 작성
-- 만약에 SALALRY로 ORDER BY 에서 정렬하길 원한다면 3을 작성
...
-- SELECT 절에 작성된 컬럼을 그대로 딸가 작성한 경우가 많음 **--

/* ORDER BY 절에서 별칭 사용하기*/
--> SELECT 절 해석 이후 ORDER BY절이 해석되기 때문에
--  SELECT 절에서 해석된 별칭을 ORDER BY 절에서 사용할 수 있음
-- EMPLOYEE 테이블에서 이름, 연봉을 내림차순 조회
SELECT EMP_NAME, SALARY * 12 AS "연봉"
FROM EMPLOYEE
...
-- ORDER BY 는 결과를 가지고 결과 정리를 하는 표현 방법
