/*
 ...
 NUMBER : 숫자형 (정수, 실수)
    실수 자리 : 소수자리 몇 번째 까지 가능한지
    NUMBER (2,2) → 0.00 ~ 99.99 까지 가능
...
*/
 ...
 PRIMARY KEY (기본키)
    ...
    - NOT NULL + UNIQUE ( 한 행에서 COLUMN 에서 똑같은 모양의 데이터는 존재해선 안됨)
 ...
 */
 ...
 모두 동일하게 사본 만들기
 CREATE TABLE 새로운테이블이름 AS SELECT * FROM 복사할테이블;
...
-- 테이블을 만들때 다른 테이블과 동일하게(컬럼, 데이터값) 복사 붙여넣기 진행할 때는
-- CREATE TABLE 테이블명
...
-- EMPLOYEE2가 EMPLOYEE 를 제대로 복사했는지 확인
