...
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY = (SELECT MAX(SALARY) FROM EMPLOYEE E WHERE DEPT_CODE = E.DEPT_CODE);

-- 3. D5 부서의 평균 급여보다 높은 급여를 받는 직원들의 이름 조회
---- SELECT EMP_NAME            AVG
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE E WHERE DEPT_CODE = 'D5');

-- 4. 급여가 300만원 이상인 직원 중에서 가장 낮은 급여를 받는 직원의 이름과 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY = (SELECT MIN(SALARY) FROM EMPLOYEE E WHERE SALARY >= 3000000);

--> 서브쿼리를 통해서 특정 조건을 만족하는 최저 급여를 찾고,
----> 조건에 맞는 급여를 받는 직원를 외부 쿼리에서 조회
...
---- SELECT 서브쿼리
---- WHERE 서브쿼리
---- 외부 SELECT DEPT_TITLE FROM DEPARTMENT WHERE >= 5;
SELECT DEPT_TITLE,
    (SELECT COUNT(*) FROM EMPLOYEE E
     WHERE E.DEPT_CODE = D.DEPT_ID) AS "직원수"
FROM DEPARTMENT D
