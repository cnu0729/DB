...
→ WHERE 절에서는 집계 함수 (SUM, AVG, MAX, MIN, COUNT 등) 사용할 수 없음
   집계 함수 (SUM, AVG, MAX, MIN, COUNT 등)는 HAVING 이나 WHERE GROUP 을  
   다른 방식으로 사용해서 작성
*/
...
-- 3. 각 부서별 최소 급여 조회 최소급여 기준 오름차순으로 정렬
SELECT DEPT_CODE, MIN(SALARY) AS "최소급여"
FROM EMPLYOEE
GROUP BY DEPT_CODE
ORDER BY "최소급여" ASC;

-- 4. 각 부서별 보너스율 합계 조회 AS "보너스율"
-- SUM 총 보너스율 기준으로 내림차순
SELECT DEPT_CODE, SUM(BONUS) AS "보너스율"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY "보너스율" DESC;
...
-- 8. 급여가 300 이상 인 사원들의 부서별 사원수 조회
...
-- COUNT AS "사원수 " 내림차순
SELECT JOB_CODE , COUNT(SALARY) AS "사원수"
FROM EMPLOYEE
WHERE SALARY <= 4000000
GROUP BY JOB_CODE
ORDER BY "사원수" DESC;

-- 11. 급여가 500 이상인 사람들의 부서별 평균 급여 조회
-- AVG AS "평균 급여" DESC
