...
SELECT DEPT_CODE, JOB_CODE, HIRE_CODE, COUNT(*) AS "사원수"
FROM EMPLOYEE
...
-- 부서 코드 오름차순 정렬
SELECT FLOOR(AVG(SALARY)) AS "급여평균", DEPT_CODE
FROM EMPLOYEE
...
ORDER BY DEPT_CODE; -- ASC생략

-- 3. 부서별 직급별 급여 평균
-- DEPT_CODE, JOB_CODEM AVG(SALARY)
-- ORDER BY DEPT_CODE 오름차순
SELECT DEPT_CODE, JOB_CODE, FLOOR(AVG(SALARY)) AS "급여평균"
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_CODE, JOB_CODE)
ORDER BY DEPT_CODE ASC;
